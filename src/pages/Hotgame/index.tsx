...
interface HotgameProps {
  hotlist: any[],
  getHomeDataActionDispatch: () => void,
  loading: boolean,
}

const Hotgame:React.FC<HotgameProps> = (props) => {
  const {
    hotlist,
    loading
  } = props
  ...
  const renderTitle = (
    <div className="ListPage-contentTitle">热门游戏</div>
)
  const videoElements = hotlist.map(
    (video: any) => {
        return (
            <VideoItem
                video={video}
...
const mapStateToProps = (state: rootState) => ({
  hotlist: state.homedata.hotlist,
...
export default connect(mapStateToProps, mapDispatchToProps)(memo(Hotgame))
